...
export function formatRunComment(
  ...
  const detailsBlocks = results
    ...
    .map((r) => formatSingleTargetComment(r, context, targetRemote))
    .join("\n\n");
...
export function formatSingleTargetComment(
  ...
  // Remote the target branch is read from — `downstream` for a downstream
  // backport, otherwise `origin`.
  targetRemote: string = "origin",
): string {
  ...
  if (error instanceof CreatePRError) {
    ...
    let manualBlock = "";
    if (error.details) {
      const { owner, repo, title, body, head, base } = error.details;
      const createPRUrl = `https://github.com/${owner}/${repo}/compare/${base}...${head}?expand=1&title=${encodeURIComponent(
        title,
      )}&body=${encodeURIComponent(body)}`;
      const ghCliCommand = `gh pr create --repo ${owner}/${repo} --base ${base} --head ${head} --title "${title.replace(
        /"/g,
        '\\"',
      )}" --body "${body.replace(/"/g, '\\"')}"`;
      manualBlock = dedent`\n\nThe backport branch was pushed, so you can create the PR manually:
    ...
    return wrapDetails(
      ...
      dedent`Backport branch was created, but creating the pull request failed with status ${error.status}.${responseLine}
             ...
             See the workflow run logs for the full response.${manualBlock}`,
    );
  }
