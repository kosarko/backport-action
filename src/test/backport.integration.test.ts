...
describe("Backport.run() orchestration", () => {
  ...
  describe("downstream", () => {
    // This fork's downstream support targets repos you don't own: fetch/checkout
    // from the downstream remote, but push the backport branch to your own fork
    // (origin) and open a cross-fork PR (head `owner:branch`).
    it("downstream repo: fetches from 'downstream', pushes to 'origin', opens a cross-fork PR", async () => {
      const github = new FakeGithub();
      const git = createMockGit();
      const config = makeConfig({
        experimental: {
      ...
      // Target branch is read from the downstream repo...
      expect(git.fetch).toHaveBeenCalledWith("main", "/tmp", 1, "downstream");
      // ...but the backport branch is pushed to our fork (origin).
      expect(git.push).toHaveBeenCalledWith(
        expect.any(String),
        "origin",
        "/tmp",
      );
      // The PR is created against the downstream repo, from our fork's branch.
      expect(github.createdPRs).toHaveLength(1);
      expect(github.createdPRs[0]).toMatchObject({
        owner: "downstream-owner",
        repo: "downstream-repo",
        head: "test-owner:backport-42-to-main",
        base: "main",
      });
    });
  });
