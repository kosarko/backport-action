...
describe("formatSingleTargetComment", () => {
  ...
  it("uses the given target remote in the manual instructions (downstream backport)", () => {
    const out = formatSingleTargetComment(
      failed(
        "main",
        new CherryPickError("cherry-pick failed", "backport-42-to-main", [
          "abc",
        ]),
      ),
      context,
    ...
    // Target branch is fetched/based on the downstream remote, not origin.
